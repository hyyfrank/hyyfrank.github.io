...
/** Fuse score: 0 = best; drop weak fuzzy hits (e.g. "docker" → "document") */
const MAX_SCORE = 0.42;
...
const buildFuseOptions = () => {
    ...
    const keys = params.fuseOpts.keys ?? defaultKeys;
    ...
    return {
        ...
        keys: Array.isArray(keys) && typeof keys[0] === 'string'
            ? defaultKeys
            : keys,
...
const filterResults = (results) => {
    ...
    return results.filter((r) => (r.score ?? 1) <= MAX_SCORE);
};
...
const performSearch = () => {
    ...
    const limit = params.fuseOpts?.limit ?? 15;
    const raw = fuse.search(query, { limit: limit * 3 });
    renderResults(filterResults(raw).slice(0, limit));
};
